...
const TEXTS = {
  ja: {
    ...
    joinButton: "仲間に入る",
    guestButton: "まずは見てみる",
    guestSubtext: "登録なしで投稿を覗けます",
    guestBanner: "👀 ゲストモードで閲覧中（登録なし）",
    guestBannerAction: "登録して仲間に入る🌺",
    guestLimitMessage: "🔒 投稿・返信・いいねには登録が必要です",
    guestSeeMore: "💌 続きを見る・参加するには",
    guestSeeMoreButton: "登録して仲間に入る（無料・30秒）🌺",
  ...
  en: {
    ...
    catchphrase: "An app to help you survive\nyour first 3 days in Malaysia",
    description: "A safe place for Japanese students\nstudying in Malaysia to connect",
    badges: ["Approval", "Word Filter", "Reporting"],
    joinButton: "Join Us",
    guestButton: "Browse First",
    guestSubtext: "Peek inside without signing up",
    guestBanner: "👀 Browsing as Guest (no account)",
    guestBannerAction: "Sign up & Join 🌺",
    guestLimitMessage: "🔒 Sign up to post, reply, or like",
    guestSeeMore: "💌 To see more & participate",
    guestSeeMoreButton: "Sign up & Join (Free · 30 sec) 🌺",
...
export default function App() {
  ...
  const handleGuestEnter = () => {
    setIsGuest(true);
    setCurrentUser({ name: "ゲスト", tier: "guest" });
    fetchPosts();
    setScreen("home");
  };
  ...
  const filteredPostsBeforeLimit = filterTag
    ? visiblePosts.filter((p) => p.tag === filterTag)
    : visiblePosts;
  // ゲスト時は最新5件まで
  const filteredPosts = isGuest ? filteredPostsBeforeLimit.slice(0, 5) : filteredPostsBeforeLimit;
  const isGuestLimitReached = isGuest && filteredPostsBeforeLimit.length > 5;
        ...
        <div style={{ position: "relative", zIndex: 1, width: "100%" }}>
          ...
          <div
            style={{
              ...
              lineHeight: 1.6,
              whiteSpace: "pre-line",
              boxShadow: "0 2px 12px rgba(249,115,22,.15)",
            }}
          >
            ✈️ {t.catchphrase}
          </div>
    ...
    <div style={{ ...WRAP, paddingBottom: 100 }}>
            ...
            <div style={{ fontSize: 10, color: "rgba(255,255,255,.4)", display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <span
                style={{
                  ...
                  padding: "1px 6px",
                  borderRadius: 4,
                  background: isGuest ? "rgba(96,165,250,.2)" : (currentUser?.verified ? "rgba(16,185,129,.2)" : "rgba(245,158,11,.2)"),
                  color: isGuest ? "#60a5fa" : (currentUser?.verified ? "#34d399" : "#fbbf24"),
                  fontWeight: 600,
                }}
              >
                {isGuest ? "👀 ゲスト" : (currentUser?.verified ? "🟢 本登録" : "🟡 仮登録")}
              </span>
            </div>
          </div>
          <button
      ...
      <div style={{ padding: "16px 14px", position: "relative", zIndex: 1 }} className="fadein">
        ...
        {isGuest && (
          <div
            style={{
              background: "linear-gradient(135deg,rgba(96,165,250,.18),rgba(147,197,253,.08))",
        ...
        {currentUser && !isGuest && !currentUser.verified && (
          <div
            style={{
        ...
        {/* ゲストモード時の登録誘導カード */}
        {isGuest && filteredPosts.length > 0 && (
          <div
            style={{
              marginTop: 16,
              padding: "20px 18px",
          ...
          >
            ...
            >
              {isGuestLimitReached
                ? (lang === "ja"
                    ? `他にも${filteredPostsBeforeLimit.length - 5}件の投稿があります`
                    : `${filteredPostsBeforeLimit.length - 5} more posts available`)
                : (lang === "ja"
                    ? "投稿・返信・いいねもできるようになります"
                    : "You can post, reply, and like")}
            </div>
            <button
              className="btn-press"
              onClick={handleGuestSignup}
      ...
      <button
        ...
        title={currentUser?.verified ? "" : (isGuest ? t.guestSeeMoreButton : t.cantPost)}
        style={{
          position: "fixed",
          bottom: 24,
          right: 20,
          ...
          background: currentUser?.verified
            ...
            : "rgba(100,100,100,.5)",
          border: "none",
          cursor: currentUser?.verified ? "pointer" : "pointer",
          fontSize: 24,
          color: "white",
        ...
        >
          ...
          >
            {isGuest ? (
              ...
              >
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.85)", marginBottom: 10, lineHeight: 1.6 }}>
                  {lang === "ja"
                    ? "💬 返信するには登録が必要です"
                    : "💬 Sign up to reply"}
                </div>
